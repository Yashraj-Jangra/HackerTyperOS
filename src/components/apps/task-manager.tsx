...
import { Cpu, MemoryStick, HardDrive, AlertCircle, Wifi, ServerCrash } from 'lucide-react'; // Added Wifi, ServerCrash
...
const performanceHistoryLength = 30; // Increased history length
...
const chartConfig = {
  ...
  networkDown: { label: "Net Down", color: "hsl(var(--accent))" }, // Use accent for network down
} satisfies ChartConfig

// Ridiculous total stats
...
export function TaskManager() {
  ...
  const [memoryUsage, setMemoryUsage] = useState(60); // Percentage of "available" RAM
  const [gpuUsage, setGpuUsage] = useState(40);
  const [diskUsage, setDiskUsage] = useState(80); // Percentage of disk I/O capacity
  const [networkUp, setNetworkUp] = useState(150); // Mbps up
  const [networkDown, setNetworkDown] = useState(850); // Mbps down
  const [uptime, setUptime] = useState(Date.now() - (Math.random() * 1000 * 3600 * 24 * 365 * 5)); // Start with some random uptime up to 5 years ago
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Update overall stats with more dramatic fluctuations
      ...
      setUptime(prev => prev + 1500); // Increment uptime
      ...
      setDiskHistory(prev => [...prev.slice(1), { time: now, disk: Math.round(newDisk) }]);
      setNetworkHistory(prev => [...prev.slice(1), { time: now, up: Math.round(newNetworkUp), down: Math.round(newNetworkDown) }]);

      // Update processes (make some fluctuate slightly)
      setProcesses(prevProcesses =>
        prevProcesses.map(p => {
          let newCpuUsage = p.cpu;
          let newMemUsage = parseFloat(p.memory.split(' ')[0]); // Get numeric memory
          ...
          if (p.name !== 'System Idle Process' && p.name !== 'WindowsUpdate.exe' && !p.name.includes('InfiniteLoop')) {
            ...
            newMemUsage = Math.max(0.01, newMemUsage + (Math.random() * (newMemUsage * 0.2) - (newMemUsage * 0.1))); // Fluctuate memory %
            ...
            newNetworkUsage = Math.max(0, newNetworkUsage + Math.random() * 5 - 2.5);
             ...
             newMemUsage = 0.008; // Keep idle memory low
          } else if (p.name.includes('InfiniteLoop')) {
              newCpuUsage = 50 + Math.random() * 10 - 5; // Keep loop CPU high
          } else if (p.name === 'WindowsUpdate.exe') {
             newCpuUsage = 99; // Always 99
          ...
          let newStatus = p.status;
           ...
           } else if (Math.random() > 0.05 && p.name !== 'WindowsUpdate.exe' && !p.status?.includes('expertly') && !p.status?.includes('Hard')) { // Keep some statuses sticky
                 newStatus = undefined; // Clear status sometimes
           }


          // Format memory back with units (simplistic)
          let memUnit = ' MB';
          if (newMemUsage > 1024) { newMemUsage /= 1024; memUnit = ' GB'; }
          if (newMemUsage > 1024) { newMemUsage /= 1024; memUnit = ' TB'; }
           if (newMemUsage > 1024) { newMemUsage /= 1024; memUnit = ' PB'; } // Absurd levels

          return {
              ...p,
              cpu: Math.round(newCpuUsage),
              memory: `${newMemUsage.toFixed(1)}${memUnit}`,
              disk: `${newDiskUsage.toFixed(1)} MB/s`, // Assume MB/s for disk
              network: `${newNetworkUsage.toFixed(1)} Mbps`, // Assume Mbps for network
              status: newStatus
          };
  ...
  }, [cpuUsage, memoryUsage, gpuUsage, diskUsage, networkUp, networkDown]); // Add new states to dependency array
  ...
  return (
    <div className="h-full flex flex-col text-foreground text-xs">
      {/* Performance Overview */}
       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-1 p-1 border-b border-primary/30">
        <Card className="bg-card/50 border-primary/20">
          ...
          <CardContent className="pb-1 px-1">
             <ChartContainer config={chartConfig} className="h-[35px] w-full">
                <ChartLineChart accessibilityLayer data={cpuHistory} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator />} />
                  <ChartLine dataKey="cpu" type="monotone" stroke="var(--color-cpu)" strokeWidth={1.5} dot={false} />
                </ChartLineChart>
              </ChartContainer>
          </CardContent>
        </Card>
        <Card className="bg-card/50 border-primary/20">
          ...
          <CardContent className="pb-1 px-1">
              <ChartContainer config={chartConfig} className="h-[35px] w-full">
                <ChartLineChart accessibilityLayer data={memoryHistory} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator />} />
                  <ChartLine dataKey="memory" type="monotone" stroke="var(--color-memory)" strokeWidth={1.5} dot={false} />
                </ChartLineChart>
              </ChartContainer>
          </CardContent>
        </Card>
         <Card className="bg-card/50 border-primary/20">
           ...
           <CardContent className="pb-1 px-1">
               <ChartContainer config={chartConfig} className="h-[35px] w-full">
                <ChartLineChart accessibilityLayer data={diskHistory} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator />} />
                  <ChartLine dataKey="disk" type="monotone" stroke="var(--color-disk)" strokeWidth={1.5} dot={false} />
                </ChartLineChart>
              </ChartContainer>
           </CardContent>
         </Card>
         <Card className="bg-card/50 border-primary/20">
           ...
           <CardContent className="pb-1 px-1">
               <ChartContainer config={chartConfig} className="h-[35px] w-full">
                <ChartLineChart accessibilityLayer data={gpuHistory} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator />} />
                  <ChartLine dataKey="gpu" type="monotone" stroke="var(--color-gpu)" strokeWidth={1.5} dot={false} />
                </ChartLineChart>
              </ChartContainer>
           </CardContent>
         </Card>
          <Card className="bg-card/50 border-primary/20">
           ...
           <CardContent className="pb-1 px-1">
               <ChartContainer config={chartConfig} className="h-[35px] w-full">
                <ChartLineChart accessibilityLayer data={networkHistory} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator />} />
                  <ChartLine dataKey="up" type="monotone" stroke="var(--color-networkUp)" strokeWidth={1.5} dot={false} />
                   <ChartLine dataKey="down" type="monotone" stroke="var(--color-networkDown)" strokeWidth={1.5} dot={false} />
                </ChartLineChart>
              </ChartContainer>
           </CardContent>
         </Card>
      ...
      <ScrollArea className="flex-grow">
        <Table className="w-full text-[11px]"> {/* Slightly smaller text */}
          ...
          <TableBody>
            {processes.map((proc) => (
              <TableRow key={proc.pid} className="hover:bg-secondary/30 border-b-primary/10 h-6">
                ...
                <TableCell className="p-1 font-medium truncate max-w-[200px]" title={proc.name}>{proc.name}</TableCell>
